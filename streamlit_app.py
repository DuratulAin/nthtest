...
if st.button('Predict'):
    ...
    st.markdown(f'<font size="5"><b>{linear_reg_prediction[2]} g/dL</b></font>', unsafe_allow_html=True)

    st.markdown('**Decision Tree Model:**')
    st.markdown(f'<font size="5"><b>{decision_tree_prediction[2]} g/dL</b></font>', unsafe_allow_html=True)

    st.markdown('**Linear Regression Model with UMAP:**')
    st.markdown(f'<font size="5"><b>{linear_reg_umap_pred[2]:.1f} g/dL</b></font>', unsafe_allow_html=True)

    st.markdown('**Decision Tree Model with UMAP:**')
    st.markdown(f'<font size="5"><b>{decision_tree_umap_pred[2]} g/dL</b></font>', unsafe_allow_html=True)
